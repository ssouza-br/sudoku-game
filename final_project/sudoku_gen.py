...
for j in range(10):
    ...
    for i in range(9):
        db.execute("INSERT INTO new_games (GAME_NUMBER, LINE, COL1, COL2, COL3, COL4, COL5, COL6, COL7, COL8, COL9) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", j+1, i+1, sudo[i][0], sudo[i][1], sudo[i][2], sudo[i][3], sudo[i][4], sudo[i][5], sudo[i][6], sudo[i][7], sudo[i][8])
        db.execute("INSERT INTO answer_games (GAME_NUMBER, LINE, COL1, COL2, COL3, COL4, COL5, COL6, COL7, COL8, COL9) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   j+1, i+1, sudo_a[i][0], sudo_a[i][1], sudo_a[i][2], sudo_a[i][3], sudo_a[i][4], sudo_a[i][5], sudo_a[i][6], sudo_a[i][7], sudo_a[i][8])
